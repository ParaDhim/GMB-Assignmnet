...
with(final, plot(log2FoldChange, -log10(pvalue), pch = 20, main = "2021482_ParasDhiman_DEGs"))
ff <- subset(final, pvalue < .05 & log2FoldChange > 1)
ff2 <- ff[1:50,]
write.csv(ff2,"D:\\iiit delhi\\sem 3\\Assignment\\GMB\\ASSMT2\\Upregulated_Top50_ParasDhiman.csv",row.names = TRUE)

ff1 <- subset(final, pvalue < .05 & log2FoldChange < -1)
ff3 <- ff1[1:50,]
write.csv(ff3,"D:\\iiit delhi\\sem 3\\Assignment\\GMB\\ASSMT2\\Downregulated_Top50_ParasDhiman.csv",row.names = TRUE)
